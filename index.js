...
function getLastCarInfo(carList) {
  return `This is a ${carList[carList.length - 1].car_make} ${carList[carList.length - 1].car_model}`;
}
...
function getCarInfoById(array, id) {
  ...
  for (let i = 1; i < array.length; i++) {
    if (array[i].id === id) ind += i;
  }
  return `This is a ${array[ind].car_make} ${array[ind].car_model}`;
}
